...
namespace Groupe12Exam
{
    public partial class Admin : Form
    {
        ...
        {
            InitializeComponent();
            tabPage1.Text = "Utilisateurs";
            tabPage2.Text = "Classes";
            tabPage3.Text = "Cours";
            tabPage4.Text = "Professeurs";
            tabPage5.Text = "Notes";
            tabPage6.Text = "Rapports";
        ...
        private void cb_role_user_SelectedIndexChanged(object sender, EventArgs e)
        {
        ...
        private void ChargerUtilisateur()
        {
            using (var db = new DbScolaire())
            {
                var user = db.TUtilisateurs
                                 .ToList();
                dgv_user.DataSource = user;
            }
        }
        // Clear cham utilisateur
        private void clearUtilisateur()
        {
            cb_role_user.Text = "Select";
            tb_nom_user.Text = "";
            tb_pass_user.Text = "";
            tb_tel_user.Text = "";
        }
        // Charger les champs de la ligne 
        private void dgv_user_click(object sender, EventArgs e)
        {
            if (dgv_user.SelectedRows.Count > 0)
            {
                // Récupérer l'ID de l'utilisateur à partir de la ligne sélectionnée
                int userId = Convert.ToInt32(dgv_user.SelectedRows[0].Cells["Id"].Value);

                // Charger les informations de l'utilisateur dans les champs
                using (var db = new DbScolaire())
                {
                    var user = db.TUtilisateurs.FirstOrDefault(u => u.Id == userId);

                    if (user != null)
                    {
                        tb_nom_user.Text = user.NomUtilisateur;
                        tb_pass_user.Text = user.MotDePasse;
                        cb_role_user.SelectedItem = user.Role;
                        tb_tel_user.Text = user.Telephone;
                    }
                }
            }
        }
        ...
        private void bt_update_user_Click(object sender, EventArgs e)
        {
            using (var db = new DbScolaire())
            {
                ...
                if (user != null)
                {
                    ...
                    user.NomUtilisateur = tb_nom_user.Text;
                    user.MotDePasse = tb_pass_user.Text;
                    user.Role = cb_role_user.SelectedItem.ToString();
                    user.Telephone = tb_tel_user.Text;

                    db.SaveChanges();  // Enregistre les modifications dans la base de données

                    ChargerUtilisateur();  // Rafraîchit le DataGridView avec les utilisateurs
                    clearUtilisateur();  // Efface les champs de saisie
                }
                else
                {
        ...
        private void bt_update_etud_Click(object sender, EventArgs e)
        {
            if (dgv_etud.SelectedRows.Count > 0)
            {
        ...
        private void bt_delete_etud_Click(object sender, EventArgs e)
        {
            if (dgv_etud.SelectedRows.Count > 0)
            {
